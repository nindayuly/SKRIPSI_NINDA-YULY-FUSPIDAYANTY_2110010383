...
// Klausa WHERE yang sama persis dengan halaman laporan
$where_clause = "WHERE ps.status_pesanan = 'selesai' AND DATE(ps.tanggal_pesanan) BETWEEN '$tanggal_mulai' AND '$tanggal_akhir'";

$sql_laporan = "SELECT 
                    p.kode_produk, p.nama_produk, kp.nama_kategori,
                    SUM(pd.jumlah) as total_terjual, p.stok
    ...
    <table class="data-table">
        ...
        <tbody>
            <?php if (!empty($laporan_data)): $no = 1; ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="5" class="text-center">Tidak ada data penjualan untuk periode ini.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
...
$mpdf = new mPDF('c', 'A4-P'); // Potrait cukup untuk laporan ini
$mpdf->WriteHTML($html);
$mpdf->Output('laporan-produk-terlaris.pdf', 'I');
exit;
?>