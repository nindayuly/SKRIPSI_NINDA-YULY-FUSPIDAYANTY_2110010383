...
if (!empty($search)) {
    ...
    $where_clause = "AND (nama_lengkap LIKE '%$search_safe%' OR email LIKE '%$search_safe%' OR telepon LIKE '%$search_safe%' OR alamat LIKE '%$search_safe%')";
}

$sql_laporan = "SELECT nama_lengkap, email, telepon, alamat, tanggal_daftar FROM pengguna WHERE peran = 'pelanggan' $where_clause ORDER BY nama_lengkap ASC";
...
<body>
    ...
    <div class="judul-laporan">
        ...
        <span>Tanggal Cetak: <?= tgl_indo(date('Y-m-d')); ?></span>
    </div>
        ...
        <tbody>
            <?php if (!empty($laporan_data)): $no = 1; ?>
                ...
                <tr>
                    ...
                    <td><?= sanitize($data['telepon'] ?? '-'); ?></td>
                    <td><?= sanitize($data['alamat'] ?? '-'); ?></td>
                </tr>
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="5" style="text-align:center;">Tidak ada data pelanggan yang cocok dengan kriteria.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
...
$mpdf->WriteHTML($html);
$mpdf->Output('laporan-pelanggan.pdf', 'I');
exit;
?>